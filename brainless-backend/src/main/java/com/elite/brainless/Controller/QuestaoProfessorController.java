...
public class QuestaoProfessorController {
    ...
    public ResponseEntity<String> saveQuestao(@RequestBody @Valid Questao data) {
        Questao questao = new Questao(data.getAlternativas(), data.getEnunciado(), data.getJustificativa(), data.getResposta(), data.getTema(), data.getIsPublica(), data.getSemestre(), data.getTurma(), data.getAtividade(), data.isSubjetiva());
