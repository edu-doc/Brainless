...
const CadastrarQuestao = () => {
  ...
  const [isSubjetiva, setIsSubjetiva] = useState("true");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (tipo === "Subjetiva") {
        ...
        setIsSubjetiva("false");
      }

      console.log(tipo);
      console.log(isSubjetiva);
      ...
      if (isEditing) {
        ...
        res = await api.put("questaoProfessor", payload);
      } else {
        const payload = {
          alternativas,
          enunciado,
        ...
        res = await api.post("questaoProfessor", payload);
      }
      ...
      if (res.status === 201) {
        ...
        setMessage(
          res.data.message || "Falha ao realizar o cadastro. Tente novamente."
        );
      }
    } catch (error: any) {
      setMessage(
    ...
    <>
      ...
      <div className="flex justify-center min-h-screen bg-gradient-to-b from-[#007BFF] to-[#0056B3]">
        <form
          ...
          className="grid grid-rows-6 gap-4 w-9/12 mt-4"
        >
          {/* Tema e Tipo */}
          <div className="flex grid-cols-2 row-span-1 gap-4 justify-between items-center">
            <div className="w-2/6">
              <label
                className="block text-black text-xl mb-2 font-bold"
                htmlFor="tema"
              >
                Tema*:
              </label>
